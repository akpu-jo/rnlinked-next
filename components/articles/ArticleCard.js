    ...
    <div className=" mx-4 py-3 border-b border-slate-100">
      <article className=" grid grid-cols-6 gap-2 bg-slate-60 py-1">
        <Link
          ...
          scroll={false}
          className=" leading-6 tracking-normal font-semibold text-slate-900 font-serif col-span-4"
        >
          {parse(article.title)}
        </Link>
      ...
      {user && showAtions && (
        <div className=" flex justify-between items-center z-10">
          <div className={` flex items-center `}>
            <Link href={`/${article.author.username}`} className=" flex justify-start items-center z-10">
                <Avatar
                  icon={
